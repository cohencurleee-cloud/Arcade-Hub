(()=>{
  ...
  const UNLOCK='arcadeAdminUnlocked';
  const SETTINGS='arcadeAdminSettings';
  const CODE='1299';
  ...
  function save(s){
    ...
    window.dispatchEvent(new CustomEvent('arcade-admin-change',{detail:{settings:s}}));
  }
...
#arcadeAdminPanel h2{margin:0 0 4px;font-size:22px}
#arcadeAdminPanel .muted{color:#9da6ba;margin-bottom:16px}
...
#arcadeAdminPanel input[type=text]{width:112px;border:1px solid #343d52;background:#090c12;color:#fff;border-radius:10px;padding:9px 8px;font:800 16px system-ui;text-align:center}
...
#arcadeAdminPanel .speedCustom{display:flex;gap:7px;align-items:center}
#arcadeAdminPanel .speedCustom button{padding:9px 10px}
...
#arcadePin .buttons{display:grid;grid-template-columns:1fr 1fr;gap:9px}
#arcadePin button{padding:11px;border-radius:12px;border:1px solid #343d52;background:#1b2230;color:#fff;font-weight:800}
...
#arcadeAIPanel .aiHead{padding:14px 14px 12px;border-bottom:1px solid #242c3b;display:flex;justify-content:space-between;align-items:center}
#arcadeAIPanel .aiHead b{font-size:17px}
#arcadeAIPanel .aiHead button{border:0;background:#1a2230;color:#fff;border-radius:10px;padding:7px 10px;font-weight:800}
#arcadeAIMessages{flex:1;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:9px}
#arcadeAIMessages .msg{max-width:86%;padding:10px 12px;border-radius:15px;line-height:1.38;white-space:pre-wrap;word-wrap:break-word}
#arcadeAIMessages .user{align-self:flex-end;background:#eef3ff;color:#111827}
#arcadeAIMessages .bot{align-self:flex-start;background:#1a2230;border:1px solid #2d374a}
#arcadeAIPanel .aiComposer{display:flex;gap:8px;padding:10px;border-top:1px solid #242c3b;background:#0c1119}
#arcadeAIPanel textarea{flex:1;resize:none;min-height:44px;max-height:110px;border:1px solid #30394d;border-radius:13px;background:#151b27;color:#fff;padding:11px;font:16px system-ui;outline:none}
#arcadeAIPanel .send{border:1px solid #4a5873;background:#e9efff;color:#101522;border-radius:13px;padding:0 15px;font-weight:900}
#arcadeAIPanel .send:disabled{opacity:.5}
...
`;
  ...
  function mountButton(){
    ...
    button.id='arcadeAdminBtn';
    button.textContent='ADMIN';
    button.onclick=openPanel;
    document.body.appendChild(button);
  }

  function autoRow(id,title,desc,checked){
    return `<div class="row"><span><b>${title}</b><small>${desc}</small></span><input id="${id}" type="checkbox" ${checked?'checked':''}></div>`;
  }
  ...
  function openPanel(){
    ...
    panel.innerHTML=`<div class="box">
      <h2>Admin Panel</h2>
      <div class="muted">Unlocked for every Arcade Hub game.</div>
      <div class="row"><b>God mode</b><input id="aaGod" type="checkbox" ${s.god?'checked':''}></div>
      <div class="row"><span><b>Game speed</b><small>Slider is a quick 0.5x–20x control.</small></span><span><input id="aaSpeed" type="range" min="0.5" max="20" step="0.5" value="${sliderCurrent}"> <b id="aaSpeedText">${speedText(current)}</b></span></div>
      <div class="row"><span><b>Custom speed</b><small>No maximum — enter any positive finite number.</small></span><span class="speedCustom"><input id="aaCustomSpeed" type="text" inputmode="decimal" autocomplete="off" spellcheck="false" value="${current}"><button id="aaSetSpeed">Set</button></span></div>
      ${autoRows}
      <div class="buttons"><button id="aaBoost">Boost</button><button id="aaReset">Reset game</button><button id="aaClose">Close</button><button id="aaLock">Lock admin</button></div>
    </div>`;
    ...
    const applySpeed=value=>{
      ...
      speed.value=String(clamp(v,.5,20));
    };
  ...
  function mountAI(){
    ...
    aiButton.id='arcadeAIButton';
    aiButton.textContent='AI';
    aiButton.setAttribute('aria-label','Open Arcade AI');
    aiButton.onclick=openAI;
    document.body.appendChild(aiButton);
  }

  function loadVibes(){
    if(document.querySelector('script[data-arcade-vibes]'))return;
    const s=document.createElement('script');
    s.dataset.arcadeVibes='1';
    s.src=new URL('arcade-vibes.js',ADMIN_SRC).href;
    document.head.appendChild(s);
  }
