(()=>{
  ...
  let activeVoices=0,lastChirpAt=0,lastScoreAt=0,lastUIClickAt=0;
  const MAX_VOICES=12;
  ...
  function ensureAudio(){configureIOSAudio();if(ctx)return true;try{const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return false;ctx=new AC({latencyHint:'interactive'});master=ctx.createGain();master.gain.value=.44;master.connect(ctx.destination);return true}catch{return false}}
  ...
  function wavSrc(freq,dur,type='sine'){
    ...
    const rate=16000,samples=Math.max(1,Math.floor(rate*dur)),buffer=new ArrayBuffer(44+samples*2),view=new DataView(buffer);
  ...
  function webTone(freq,dur=.10,vol=.25,type='sine',delay=0){
    if(!enabled||!ensureAudio()||activeVoices>=MAX_VOICES)return;
    const play=()=>{
      if(!ctx||ctx.state!=='running'||activeVoices>=MAX_VOICES)return;
      activeVoices++;
      const o=ctx.createOscillator(),g=ctx.createGain(),t=ctx.currentTime;
      o.type=type;o.frequency.setValueAtTime(freq,t);g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(Math.max(.001,vol),t+.006);g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.connect(g);g.connect(master);o.start(t);o.stop(t+dur+.02);
      o.onended=()=>{activeVoices=Math.max(0,activeVoices-1);try{o.disconnect();g.disconnect()}catch{}}
    };
  ...
  function tone(freq,dur=.10,vol=.45,type='sine',delay=0){
    if(!enabled)return;
    configureIOSAudio();
    if(ensureAudio()&&ctx.state==='running'){webTone(freq,dur,vol,type,delay);return}
    const play=()=>{try{const a=new Audio(wavSrc(freq,dur,type));a.preload='auto';a.volume=Math.max(.08,Math.min(1,vol*1.4));a.playsInline=true;a.play().catch(()=>{})}catch{}};
    if(delay>0)setTimeout(play,delay*1000);else play()
  }
  ...
  function birdChirp(){
    ...
    if(now-lastChirpAt<95)return;
    lastChirpAt=now;
    if(ensureAudio()&&ctx.state==='running'&&activeVoices<MAX_VOICES){
      activeVoices++;
      const t=ctx.currentTime,o=ctx.createOscillator(),g=ctx.createGain();o.type='sine';o.frequency.setValueAtTime(1450,t);o.frequency.exponentialRampToValueAtTime(2350,t+.04);o.frequency.exponentialRampToValueAtTime(1700,t+.09);g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(.36,t+.006);g.gain.exponentialRampToValueAtTime(.0001,t+.105);o.connect(g);g.connect(master);o.start(t);o.stop(t+.115);o.onended=()=>{activeVoices=Math.max(0,activeVoices-1);try{o.disconnect();g.disconnect()}catch{}}
    }
  }
  ...
  function musicBeat(){
    ...
    if(n)webTone(n,Math.max(.10,meta.beat/1000*.65),game==='hub'?.105:.09,leadType);
    if(i%8===0){const b=meta.bass[Math.floor(i/4)%meta.bass.length];webTone(b,Math.max(.16,meta.beat/1000*1.4),.065,'triangle')}
  }

  function confirmation(){tone(660,.10,.34,'sine');tone(990,.12,.30,'sine',.09)}
  ...
  function init(){
    ...
    addEventListener('pointerdown',wake,{passive:true});addEventListener('touchstart',wake,{passive:true});addEventListener('click',wake,{passive:true});
    addEventListener('pageshow',()=>{if(enabled&&!started)startAudio(false)});addEventListener('focus',()=>{if(enabled&&!started)startAudio(false)});
    ...
    window.ArcadeVibes={tone,start:startAudio,play:async name=>{
      if(!enabled)return;configureIOSAudio();await resumeAudio();
      if(name==='flap'&&game==='flappy'){birdChirp();return}
      if(name==='score'){
        const now=performance.now();if(now-lastScoreAt<120)return;lastScoreAt=now
      }
      const map={score:[1047,.06,.24],hit:[145,.13,.46],win:[1175,.16,.34],level:[880,.10,.28]};const v=map[name]||[520,.07,.22];tone(v[0],v[1],v[2],'sine')
    }};
